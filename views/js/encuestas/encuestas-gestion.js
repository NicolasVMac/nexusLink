...
let tipoEncuesta = atob(getParameterByName('tipoEncuesta'));
...
// const generarBotonSaveSegmento = async (procesoEncuesta, tipoEncuesta) => {

//     contenedorBotonSegmento.innerHTML = '';

//     let botonSave = document.createElement('button');
//     botonSave.type = 'submit';
//     botonSave.id = 'btnSaveEncuestaSegmento';
//     botonSave.className = "btn btn-success ms-2";
//     botonSave.innerHTML = '<i class="fas fa-clipboard-list"></i> Terminar Encuesta';
//     botonSave.onclick = async () => {
//         await obtenerCalifacionEncuesta(procesoEncuesta, tipoEncuesta);
//     };

//     contenedorBotonSegmento.appendChild(botonSave);
...
const mostrarInfoEncuesta = async (idEncuesta) => {
    ...
    $.ajax({
        ...
        success:function(respuesta){
            ...
            $("#textModalidadConsulta").html(respuesta["modalidad_consulta"]);
            $("#textTipoEncuesta").html(respuesta["tipo_encuesta"]);
...
const obtenerProcesosInicialesEncuesta = async (tipoEncuesta) => {
...
const generarTablaParaclinicos = async (tipoEncuesta) => {
...
const generarTablaSegmentos = async (bodyContent, id_encu_proceso, proceso) => {
    ...
    for(const segmentoEncuesta of segmentos){
        ...
        const arrayParaclinicos = [4,59,123,124];
        ...
        if(!arrayParaclinicos.includes(parseInt(id_encu_segmento))){
            ...
            if(id_encu_segmento == 3 || id_encu_segmento == 58){
    ...
    if(isTableParaClinicos){
        await generarTablaParaclinicos(tipoEncuesta);
    }
...
const eliminarProfesional = (idEncuProcesoGestion, idEncuProceso, proceso) => {

    Swal.fire({
        title: `¿Desea eliminar el Profesional: ${proceso}?`,
        icon: 'question',
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        cancelButtonText: "Cancelar",
        ...
        if (result.isConfirmed){
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    'proceso': 'eliminarProfesional',
                    'idEncuProcesoGestion':idEncuProcesoGestion,
                    'idEncuProceso':idEncuProceso,
                    'idEncuesta':idEncuesta
                },
                success:function(respuesta){
                    ...
                    if(respuesta == 'ok'){
                        ...
                        Swal.fire({
                            text: '¡Se Elimino el Profesional correctamente!',
                            icon: 'success',
                            confirmButtonText: 'Gestionar',
                            allowOutsideClick: false,
                            allowEscapeKey: false,
                    ...
                    }else if(respuesta == 'error-eliminando-proceso'){

                        Swal.fire({
                            title: 'Error!',
                            text: '¡El Profesional no se pudo eliminar correctamente!',
                            icon: 'error',
                            confirmButtonText: 'Cerrar',
                            allowOutsideClick: false,
                            allowEscapeKey: false,
                    ...
                    }else if(respuesta == 'error-no-permitido'){

                        Swal.fire({
                            title: 'Error!',
                            text: '¡El Profesional no se puede eliminar porque ya tiene respuestas almacenadas!',
                            icon: 'error',
                            confirmButtonText: 'Cerrar',
                            allowOutsideClick: false,
                            allowEscapeKey: false,
...
// const validarPatologiaMultiple = () => {

//     let selectPatologia = document.querySelector('.select-patologia');

//     let selectedValues = [];

//     for(let patologias of selectPatologia.options){
//         if(patologias.selected){
//             selectedValues.push(patologias.value);
//         }
//     }

//     if(selectedValues.length > 1){
//         if(selectedValues.includes('NINGUNA')){
//             Swal.fire({
//                 text: 'Si selecciona NINGUNA no puedo seleccionar otras Patologias.',
//                 icon: 'error',
//                 confirmButtonText: 'Cerrar',
//                 allowOutsideClick: false,
//                 allowEscapeKey: false
...
const validarFechaInstrumento = async (idSegmento, idPregunta) => {
    ...
    let preguntas = await obtenerPreguntasSegmentosEncuesta(idSegmento);
...
const generarPreguntasSegmentos = async (idSegmento, porcentajeGeneral) => {
    ...
    let porcentajePregunta = porcentajeSegmento / preguntas.length;
    ...
    // console.log(porcentajePregunta.toFixed(2));

    for(const pregunta of preguntas){

        // console.log(pregunta);
        // if(pregunta.tipo_pregunta == 'select-multiple'){
        ...
        if(pregunta.tipo_pregunta == 'number' || pregunta.tipo_pregunta == 'textarea' || pregunta.tipo_pregunta == 'select-patologia'){
        ...
        tableBody.innerHTML += `
            <tr>
                ...
                <td class="porcentaje-pregunta-${pregunta.id_encu_pregunta}">${porcentajePregunta.toFixed(2)}%</td>            
            </tr>
        `;

    }

    // console.log(arraySelectMultiple);
...
const generarLiGestionEncuesta = async (idEncuesta) => {
    ...
    for(const procesoEncuesta of arrayProcesosGestion) {
        ...
        if(!calificacion){
            ...
            newLi.innerHTML = `<a class="nav-link text-success" style="font-size: 12px !important;">
                <i class="fas fa-th-list"></i> ${proceso}
            </a>`;
            
        }

        // newLi.innerHTML = `<a class="nav-link text-danger" style="font-size: 12px !important;">
        //     <i class="fas fa-th-list"></i> ${proceso}
        // </a>`;


        // newLi.innerHTML = `<a class="nav-link" style="font-size: 12px !important;" idProceso="${id_encu_proceso}" tipoEncuesta="${encuesta}" id="proceso-${id_encu_proceso}-tab" data-bs-toggle="tab" href="#tab-proceso-${id_encu_proceso}" role="tab" aria-controls="tab-proceso-${id_encu_proceso}">
        //         <i class="fas fa-th-list"></i> ${proceso}
        //     </a>`;
...
const changeProcesoProfesional = async () => {

    let procesoProfesional = document.querySelector('#procesoProfesional').value;

    // console.log(procesoProfesional);
    ...
    $.ajax({
        ...
        type: 'POST',
        data: {
            'proceso': 'selectAuditoresProceso',
            'procesoProfesional':procesoProfesional,
            'tipoEncuesta':tipoEncuesta
        },
        success:function(respuesta){
        
            // console.log(respuesta);
            $("#auditorProceso").html(respuesta);
...
const mostrarInformacionAuditoriaEncuesta = async () => {
    ...
    const { nombre_paciente, edad, sexo, modalidad_consulta } = infoAuditoriaEncuesta;
    ...
    $("#modalidadConsultaPacienteEncuesta").val(modalidad_consulta);
...
tablaProcesosGestionEncuesta = $('#tablaProcesosGestionEncuesta').DataTable({

    columns: [
        { name: 'PROCESO', data: 'proceso' },
        {
            name: 'OPCIONES', orderable: false, render: function (data, type, row) {
                return `<button type="button" class="btn btn-danger btn-sm" onclick="eliminarProfesional(${row.id_encu_proce_gestion}, ${row.id_encu_proceso}, '${row.proceso}')" title="Eliminar Profesional"><i class="fas fa-trash-alt"></i></button>`;
            }
        }
    ],
    ajax: {
        ...
        type: 'POST',
        data: {
            'proceso': 'listaProcesosGestionEncuesta',
            'idEncuesta': idEncuesta
        }
    }
