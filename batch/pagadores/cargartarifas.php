...
$archivoTarifario = Connection::connectBatch()->prepare("SELECT * FROM pagadores_pagadores_tarifas_archivos_masivo WHERE estado = 'CARGAR' ORDER BY fecha_crea ASC LIMIT 1");
...
if(empty($archivoTarifario)){
    ...
    $cargando= Connection::connectOnly()->prepare("UPDATE pagadores_pagadores_tarifas_archivos_masivo set estado = 'CARGANDO', fecha_ini_carga = CURRENT_TIMESTAMP WHERE id_archivo_tarifas = $idBase"); 
    $cargando->execute();
    ...
    for($i = 2; $i <= $NumFilas; $i++){
        ...
        $tabla = "pagadores_pagadores_tarifas";

        $crearTarifa = ModelPagadores::mdlCrearTarifaUnitaria($datos);
    ...
    if(sizeof($erroresCarga)>0){
        ...
        $rutaErrores = "../../../archivos_nexuslink/pagadores/pagadores_archivos_tarifarios/errores_carga/";
        $rutaFinErros = $rutaErrores.$nombreArchivo.".txt";

        $resl_vali = fopen("../../../archivos_nexuslink/pagadores/pagadores_archivos_tarifarios/errores_carga/" . $nombreArchivo . ".txt", "w") or die("Problema al Crear Archivo"); // Archivo Escritura
        ...
        $respuesta = Connection::connectOnly()->prepare("UPDATE pagadores_pagadores_tarifas_archivos_masivo SET fecha_fin_carga = CURRENT_TIMESTAMP, estado = 'ERROR_CARGA', ruta_archivo_errors = '$rutaFinErros' WHERE id_archivo_tarifas = $idBase");
        ...
        $respuesta = Connection::connectOnly()->prepare("UPDATE pagadores_pagadores_tarifas_archivos_masivo SET fecha_fin_carga = CURRENT_TIMESTAMP, estado = 'CARGADO' WHERE id_archivo_tarifas = $idBase");
