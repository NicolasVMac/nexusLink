...
$archivoTarifario = Connection::connectOnly()->prepare("SELECT * FROM pagadores_pagadores_tarifas_archivos_masivo WHERE estado = 'SUBIDA' ORDER BY fecha_crea ASC LIMIT 1");
...
if(empty($archivoTarifario)){
    ...
    $enValidacion = Connection::connectBatch()->prepare("UPDATE pagadores_pagadores_tarifas_archivos_masivo SET estado = 'VALIDANDO', fecha_ini_valida = CURRENT_TIMESTAMP WHERE id_archivo_tarifas = $idBase");
    ...
    if(sizeof($errores)>0){
        ...
        $resl_vali = fopen("../../../archivos_nexuslink/pagadores/pagadores_archivos_tarifarios/errores/" . $nombreFin . ".txt", "w") or die("Problema al Crear Archivo"); // Archivo Escritura
        ...
        $respuesta = Connection::connectBatch()->prepare("UPDATE pagadores_pagadores_tarifas_archivos_masivo SET ruta_archivo_errors = '$rutaFinErros', estado = 'ERROR', fecha_fin_valida = CURRENT_TIMESTAMP WHERE id_archivo_tarifas = $idBase");
        ...
        ===================*/
        $respuesta = Connection::connectOnly()->prepare("UPDATE pagadores_pagadores_tarifas_archivos_masivo SET fecha_fin_valida = CURRENT_TIMESTAMP, estado = 'CARGAR' WHERE id_archivo_tarifas = $idBase");
