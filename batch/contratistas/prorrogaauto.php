...
$contratosVencidos = Connection::connectOnly()->prepare("SELECT * FROM contratistas_contratistas_contratos WHERE fecha_fin_real < NOW() AND active = 1");
...
if(empty($contratosVencidos)){
    ...
    foreach ($contratosVencidos as $key => $valueCon) {
        ...
        $getProrrogaContrato = Connection::connectOnly()->prepare("SELECT * FROM contratistas_contratista_contratos_prorrogas WHERE id_contrato = {$valueCon['id_contrato']} AND is_active = 1 AND estado = 'DISPONIBLE' ORDER BY id_prorroga ASC LIMIT 1");
        ...
        if(!empty($getProrrogaContrato)){
            ...
            $resultado = ControladorContratistas::ctrAplicarProrrogaContrato($datosProrroga);
