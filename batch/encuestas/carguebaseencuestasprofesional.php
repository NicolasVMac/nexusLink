...
$archivoBase = Connection::connectBatch()->prepare("SELECT * FROM encuestas_bases_encuestas_profesional WHERE estado = 'CARGAR' ORDER BY fecha_crea ASC LIMIT 1");
$archivoBase->execute();
...
$cargando= Connection::connectOnly()->prepare("UPDATE encuestas_bases_encuestas_profesional set estado = 'CARGANDO', fecha_ini_carga = CURRENT_TIMESTAMP WHERE id_base_encuesta = $idBase"); 
$cargando->execute();
...
if(empty($archivoBase)){
    ...
    for($i = 2; $i <= $NumFilas; $i++){
        ...
        $datosEncuestas = array(
            ...
            "modalidad_consulta_tipo_atencion" => mb_strtoupper($modalidadConsultaoTipoAtencion),
            
        );

        $tabla = "encuestas_encuestas_procesos_profesional";

        $crearEncuesta = ModelEncuestasProfesional::mdlCrearEncuesta($tabla, $datosEncuestas);
    ...
    if(sizeof($erroresCarga)>0){
        ...
        $rutaErrores = "../../../archivos_vidamedical/encuestas/archivos_bases_encuestas_profesional/errores_carga/";
        $rutaFinErros = $rutaErrores.$nombreArchivo.".txt";

        $resl_vali = fopen("../../../archivos_vidamedical/encuestas/archivos_bases_encuestas_profesional/errores_carga/" . $nombreArchivo . ".txt", "w") or die("Problema al Crear Archivo"); // Archivo Escritura

        fwrite($resl_vali, $nombreArchivo . "\n");
        fwrite($resl_vali, "Archivo generado el " . date("Y-m-d h:i:s:a") . "\n");
        fwrite($resl_vali, "Validacion Carga Pacientes Base Encuestas Profesional\n");
        ...
        $respuesta = ModelEncuestasProfesional::mdlActualizarBaseEncuestas("ERROR_CARGA", $datos);
        ...
        $respuesta = ModelEncuestasProfesional::mdlActualizarBaseEncuestas("CARGADO", $datos);
